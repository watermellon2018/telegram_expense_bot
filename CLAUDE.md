...
mypy              # проверка типов (список модулей — в [tool.mypy] files)
```

Покрытие, отдельные тесты, настройка БД — см. [TESTING.md](TESTING.md) и [SETUP_LOCAL.md](SETUP_LOCAL.md).

Линтер/типы: **ruff** и **mypy** (конфиг — `pyproject.toml`, dev-зависимости — `requirements-dev.txt`). Гоняются в CI (job `lint`) и в preCommit. Строгость намеренно **мягкая (baseline)**: ruff — набор `E,F,I` (часть кодов в ignore); mypy — нестрогий и проверяет только уже типизированные модули (`[tool.mypy] files`), легаси добавляется по мере аннотирования. Ужесточать набор правил — отдельной задачей, не точечно. Других форматтеров (black/isort/flake8) не вводим без согласования.
